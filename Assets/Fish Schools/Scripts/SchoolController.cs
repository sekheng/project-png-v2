...
public class SchoolController:MonoBehaviour{
	...
	public void Spawn(GameObject obj, Vector3 pos, Quaternion rot)
    {
		AddFish(obj,pos,rot);
		Invoke("AutoRandomWaypointPosition", RandomWaypointTime());
	}
	...
	public void AddFish(GameObject ob, Vector3 pos, Quaternion rot)
	{
		if (_groupChildToNewTransform) InstantiateGroup();
		SchoolChild obj = (Instantiate(ob) as GameObject).GetComponent<SchoolChild>();
	...
	public void AddManual(SchoolChild obj)
    {
        _roamers.Add(obj);
        AddChildToParent(obj.transform);
    }
